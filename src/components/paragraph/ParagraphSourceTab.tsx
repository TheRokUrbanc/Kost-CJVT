...
export default function ParagraphSourceTab({ data, searchType, currentLemma, sentenceId }: TabProps) {
    ...
    const generateWord = (w: Pick<Word, "id" | "ana" | "text" | "lemma">) => {
        ...
        const dynamicClasses = clsx(isLemma && "font-bold", isTarget && "text-semantic-error");
        ...
        <div className="px-4 flex gap-4 justify-between">
            <div className="flex flex-col gap-4 grow">
                <div className="bg-surface-static-secondary rounded-md p-4">
                    <p className="example text-justify">{data.Sentences.map((s) => s.Words.map(generateWord))}</p>

                    <span className="caption text-light-grey">Izvorno besedilo</span>
                </div>
