...
export default function Tab({ label, name, icon, isActive = false, onClick, disabled }: TabProps) {
    const buttonClasses = clsx(
        "bg-white px-5 group inline-flex select-none border border-static-border",
    ...
    const labelClasses = clsx(
        "text-sm tracking-wide transition-all duration-200 group-hover:text-black text-grey",
        isActive && "!text-black",
    );
